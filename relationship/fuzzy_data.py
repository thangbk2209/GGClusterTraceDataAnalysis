...
colnames = ['cpu_rate','mem','disk_io','disk_space']
...
for i in range(len(cpu_rate)):
	fuzzy_cpu_rate.append(round(cpu_rate[i],4))
	fuzzy_mem_usage.append(round(mem_usage[i],4))
	fuzzy_disk_io_time.append(round(disk_io_time[i],4))
	fuzzy_disk_space.append(round(disk_space[i],4))
...
dataFuzzyDf.to_csv('data/5_Fuzzy_Mem_sampling_617685_metric_10min_datetime_origin.csv', index=False, header=None)