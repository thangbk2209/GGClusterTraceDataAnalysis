...
for sliding in sliding_widow:
	for batch_size in batch_size_array:
		...
		plt.ylabel("CPU")
		ax.text(0,0, 'testScore-sliding=%s-batch_size=%s_optimise=sgd: %s MAE- %s RMSE- '%(sliding,batch_size, MAE, RMSE), style='italic',
		        bbox={'facecolor':'red', 'alpha':0.5, 'pad':8})
		plt.legend()
		plt.savefig('sliding=%s_batchsize=%s_optimize=sgd.png'%(sliding,batch_size))
		plt.show()
