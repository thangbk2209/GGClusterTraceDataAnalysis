...
for sliding in sliding_widow:
	for batch_size in batch_size_array:
		for optimize in optimizerArr: 
			...
			for j in range(sliding + 250, sliding + 450 ,1):
			...
			plt.ylabel("Memory")
			ax.text(0,0, 'testScore-sliding=%s-batch_size=%s_optimise=%s: %s RMSE- %s MAE'%( sliding,batch_size,optimize, RMSE,MAE), style='italic',
			        bbox={'facecolor':'red', 'alpha':0.5, 'pad':8})
			plt.legend()
			plt.savefig('sliding=%s_batchsize=%s_optimize=%s.pdf'%(sliding,batch_size, optimize))
			plt.show()
