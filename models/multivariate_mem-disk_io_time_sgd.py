...
df = read_csv('./data/5_Fuzzy_Mem_sampling_617685_metric_10min_datetime_origin.csv', header=None, index_col=False, names=colnames, engine='python')
...
disk_io_time_nomal = scaler.fit_transform(disk_io_time)
...
for sliding in sliding_widow:
	...
	trainX, trainY = data[0:train_size], mem_nomal[sliding:train_size+sliding]
	testX = data[train_size:length-sliding]
	testY =  mem[train_size+sliding:length]
	print 'trainX'
	print trainX
	print trainX[0]
	...
	for batch_size in batch_size_array: 
		...
		for k in range(5):
			...
			for optimize in optimizerArr:
				...
				# plt.show()
				plt.savefig('results/worst/multivariate/memFuzzy/testSGD/%s/history_sliding=%s_batchsize=%s_optimize=%s.png'%(modelName,sliding,batch_size,optimize))
				...
				testDf.to_csv('results/worst/multivariate/memFuzzy/testSGD/%s/testPredictInverse_sliding=%s_batchsize=%s_optimize=%s.csv'%(modelName,sliding,batch_size,optimize), index=False, header=None)
				errorScore=[]
				errorScore.append(testScoreRMSE)
				errorScore.append(testScoreMAE)
				errorDf = pd.DataFrame(np.array(errorScore))
				errorDf.to_csv('results/worst/multivariate/memFuzzy/testSGD/%s/error_sliding=%s_batchsize=%s_optimize=%s.csv'%(modelName,sliding,batch_size,optimize), index=False, header=None)
