...
dataframe = read_csv('./data/5_Fuzzy_Mem_sampling_617685_metric_10min_datetime_origin.csv', usecols=[1], engine='python')
...
for sliding in sliding_window:
	...
	for batch_size in batch_size_arr: 
		...
		# plt.show()
		plt.savefig('lastResults/univariate/memFuzzy_3layer/history_sliding=%s_batchsize=%s_optimize=sgd.png'%(sliding,batch_size))
		# make predictions
		...
		testDf.to_csv('lastResults/univariate/memFuzzy_3layer/testPredictInverse_sliding=%s_batchsize=%s_optimize=sgd.csv'%(sliding,batch_size), index=False, header=None)
		errorScore=[]
		errorScore.append(testScoreRMSE)
		errorScore.append(testScoreMAE)
		errorDf = pd.DataFrame(np.array(errorScore))
		errorDf.to_csv('lastResults/univariate/memFuzzy_3layer/error_sliding=%s_batchsize=%s_optimize=sgd.csv'%(sliding,batch_size), index=False, header=None)