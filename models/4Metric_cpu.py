...
for sliding in sliding_widow:
	...
	for i in range(length-sliding):
		...
		for j in range(sliding):
			datai.append(disk_io_normal[i+j])
		data.append(datai)
	data = np.array(data)
	...
	for batch_size in batch_size_array: 
		...
		model = Sequential()
		model.add(LSTM(1, activation = 'relu',input_shape=(1, sliding*4)))
		...
		# plt.show()
		plt.savefig('lastResults/4Metric/cpu/history_sliding=%s_batchsize=%s_optimize=sgd.png'%(sliding,batch_size))
		...
		testDf.to_csv('lastResults/4Metric/cpu/testPredictInverse_sliding=%s_batchsize=%s_optimize=sgd.csv'%(sliding,batch_size), index=False, header=None)
		errorScore=[]
		errorScore.append(testScoreRMSE)
		errorScore.append(testScoreMAE)
		errorDf = pd.DataFrame(np.array(errorScore))
		errorDf.to_csv('lastResults/4Metric/cpu/error_sliding=%s_batchsize=%s_optimize=sgd.csv'%(sliding,batch_size), index=False, header=None)
