...
for sliding in sliding_widow:
	...
	for batch_size in batch_size_array: 
		...
		model = Sequential()
		model.add(LSTM(512,return_sequences=True, activation = 'relu',input_shape=(sliding,3)))
		...
		# plt.show()
		plt.savefig('lastResults/3Metric/mem_timesteps/history_sliding=%s_batchsize=%s_optimize=sgd.png'%(sliding,batch_size))
		...
		testDf.to_csv('lastResults/3Metric/mem_timesteps/testPredictInverse_sliding=%s_batchsize=%s_optimize=sgd.csv'%(sliding,batch_size), index=False, header=None)
		errorScore=[]
		errorScore.append(testScoreRMSE)
		errorScore.append(testScoreMAE)
		errorDf = pd.DataFrame(np.array(errorScore))
		errorDf.to_csv('lastResults/3Metric/mem_timesteps/error_sliding=%s_batchsize=%s_optimize=sgd.csv'%(sliding,batch_size), index=False, header=None)
