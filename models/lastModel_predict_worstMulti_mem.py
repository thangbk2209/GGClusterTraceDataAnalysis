...
df = read_csv('./data/5_Fuzzy_Mem_sampling_617685_metric_10min_datetime_origin.csv', header=None, index_col=False, names=colnames, usecols=[1,2], engine='python')
...
for sliding in sliding_widow:
	...
	for batch_size in batch_size_array: 
		...
		model = Sequential()
		model.add(LSTM(512,return_sequences=True, activation = 'relu',input_shape=(sliding, 2)))
		...
		# plt.show()
		plt.savefig('lastResults/worst_multivariate/memFuzzy_timesteps=2/history_sliding=%s_batchsize=%s_optimize=sgd.png'%(sliding,batch_size))
		...
		testDf.to_csv('lastResults/worst_multivariate/memFuzzy_timesteps=2/testPredictInverse_sliding=%s_batchsize=%s_optimize=sgd.csv'%(sliding,batch_size), index=False, header=None)
		errorScore=[]
		errorScore.append(testScoreRMSE)
		errorScore.append(testScoreMAE)
		errorDf = pd.DataFrame(np.array(errorScore))
		errorDf.to_csv('lastResults/worst_multivariate/memFuzzy_timesteps=2/error_sliding=%s_batchsize=%s_optimize=sgd.csv'%(sliding,batch_size), index=False, header=None)
