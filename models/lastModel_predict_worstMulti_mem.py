...
for sliding in sliding_widow:
	...
	for batch_size in batch_size_array: 
		...
		# plt.show()
		plt.savefig('lastResults/worst_multivariate/memFuzzy_3layer/history_sliding=%s_batchsize=%s_optimize=sgd.png'%(sliding,batch_size))
		...
		testDf.to_csv('lastResults/worst_multivariate/memFuzzy_3layer/testPredictInverse_sliding=%s_batchsize=%s_optimize=sgd.csv'%(sliding,batch_size), index=False, header=None)
		errorScore=[]
		errorScore.append(testScoreRMSE)
		errorScore.append(testScoreMAE)
		errorDf = pd.DataFrame(np.array(errorScore))
		errorDf.to_csv('lastResults/worst_multivariate/memFuzzy_3layer/error_sliding=%s_batchsize=%s_optimize=sgd.csv'%(sliding,batch_size), index=False, header=None)
