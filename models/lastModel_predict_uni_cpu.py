...
for sliding in sliding_window:
	...
	for batch_size in batch_size_arr: 
		...
		# plt.show()
		plt.savefig('lastResults/univariate/cpuFuzzy/history_sliding=%s_batchsize=%s_optimize=sgd.png'%(sliding,batch_size))
		# make predictions
		...
		testDf.to_csv('lastResults/univariate/cpuFuzzy/testPredictInverse_sliding=%s_batchsize=%s_optimize=sgd.csv'%(sliding,batch_size), index=False, header=None)
		errorScore=[]
		errorScore.append(testScoreRMSE)
		errorScore.append(testScoreMAE)
		errorDf = pd.DataFrame(np.array(errorScore))
		errorDf.to_csv('lastResults/univariate/cpuFuzzy/error_sliding=%s_batchsize=%s_optimize=sgd.csv'%(sliding,batch_size), index=False, header=None)