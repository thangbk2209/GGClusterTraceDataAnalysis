	...
	for batch_size in batch_size_arr: 
		...
		for k in range(6):
			...
			for optimize in optimizerArr:
				...
				# plt.show()
				plt.savefig('results/univariate/memFuzzy/%s/history_sliding=%s_batchsize=%s_optimize=%s.png'%(modelName,sliding,batch_size,optimize))
				# make predictions
				...
				testDf.to_csv('results/univariate/memFuzzy/%s/testPredictInverse_sliding=%s_batchsize=%s_optimize=%s.csv'%(modelName,sliding,batch_size,optimize), index=False, header=None)
				errorScore=[]
				errorScore.append(testScoreRMSE)
				errorScore.append(testScoreMAE)
				errorDf = pd.DataFrame(np.array(errorScore))
				errorDf.to_csv('results/univariate/memFuzzy/%s/error_sliding=%s_batchsize=%s_optimize=%s.csv'%(modelName,sliding,batch_size,optimize), index=False, header=None)